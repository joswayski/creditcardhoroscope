...
func (s *Server) RateHoroscope(w http.ResponseWriter, r *http.Request) {
	...
	if !slices.Contains(validRatings, reqBody.Rating) || !strings.HasPrefix(reqBody.PaymentIntentId, "pi_") || externalId == "" {
		slog.Error("Error when parsing the request body", "error", err, "body", reqBody)
		w.WriteHeader(http.StatusBadRequest)
		json.NewEncoder(w).Encode(map[string]string{
	...
	rating = NULL AND
	external_id = $3 AND
	payment_intent_id = (SELECT id from payment_intents WHERE payment_intend_id = $4)`,
		reqBody.Rating,
		time.Now().UTC(),
		externalId,
	...
	if dbResult.RowsAffected() == 0 {
		slog.Error("Horoscope not found", "body", reqBody)
