...
impl DatabaseConfig {
    fn new() -> Result<DatabaseConfig, Vec<String>> {
        ...
        match database_url {
            Ok(database_url) => Ok(DatabaseConfig {
                min_connections: 2,
                max_connections: 10,
                database_url,
            }),
            database_url => {
                let errors = [database_url.err()].into_iter().flatten().collect();
                Err(errors)
            }
        }
    }
...
impl Config {
    pub fn new() -> anyhow::Result<Config> {
        ...
        match (
            ...
            (Ok(app), Ok(stripe), Ok(ai), Ok(server), Ok(database)) => Ok(Config {
                app,
                ai,
                stripe,
                server,
            ...
            (api_res, stripe_res, ai_res, server_res, database_result) => {
                let config_errors = [
                    api_res.err(),
                    stripe_res.err(),
                    ai_res.err(),
...
impl AppConfig {
    fn new() -> Result<AppConfig, Vec<String>> {
        ...
        match base_url {
            ...
            base_url => {
                let errors = [base_url.err()].into_iter().flatten().collect();
                Err(errors)
            }
        }
    }
...
impl AIConfig {
    fn new() -> Result<AIConfig, Vec<String>> {
        let base_url = get_non_empty("AI_BASE_URL")
            ...
            .and_then(|v| {
                url::Url::parse(&v)
                    .map_err(|e| format!("AI_BASE_URL is an invalid URL: {e}").into())
            });
